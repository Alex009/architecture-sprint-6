...
# Сетап сервера для kubemini

## Создание виртуалки на Yandex Cloud

Так как macos с m1 не умеет работать с kubemini (нет нужной виртуализации), делаем себе сервер.
...
После создания инфраструктуры смотрим куда цепляться.
В файле стейта нужно посмотреть в instance с именем `minikube-vm` в блоке `network_interface` какой адрес указан в
`nat_ip_address` - по этому адресу и нужно подключаться.
```shell
ssh debian@62.84.117.148
```
